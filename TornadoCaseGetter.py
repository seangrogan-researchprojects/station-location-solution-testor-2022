...
from nearest_insertion_attempt_2 import nearest_insertion_attempt_two
from shapely.geometry import Point
from tqdm import tqdm

from routing_algo_refresh import cluster_waypoints_with_k_means
...
class TornadoCaseGetter:
        ...
        with tqdm(total=len(self.sbw_cases), desc="Attaching waypoints to SBWs", leave=False) as pbar:
            self.sbw_cases['waypoints'] = \
                self.sbw_cases.apply(
                    lambda x: self.filter_points(__temp_waypoints, x, pbar),
                    axis=1,
                    result_type='reduce')
    ...
    def filter_points(waypoints, geom, pbar=None):
        ...
        try:
            ...
            waypoints = list(pt for pt in waypoints.index.tolist() if geom.contains(Point(pt[0], pt[1])))
        if waypoints:
            return waypoints
        try:
            _=geom.geometry.bounds
            return np.NaN
        except AttributeError:
            return list()
...
class RouteClass:
    ...
    def cluster_first_nearest_insertion_route_second(n_depots, _waypoints):
        ...
        routes, dists, t_bar = nearest_insertion_attempt_two(clusters)
