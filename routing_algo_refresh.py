...
def route_waypoints_with_nearest_insertion(waypoints, c_num=None):
    ...
    tour, dist = construct_path_nearest_insertion_heuristic_v4(dist_matrix, start_min_arc=True, c_num=c_num)
...
def construct_path_nearest_insertion_heuristic_v4(dist_matrix, start_min_arc=True, c_num=None):
    def _find_next_nearest(dist_matrix_DF, tour, avail_cities):
        next_nearest = None
        tempDM = dist_matrix_DF.loc[dist_matrix_DF.index.isin(tour[1:-1]), dist_matrix_DF.columns.isin(avail_cities)] # dist_matrix_DF.columns.isin(avail_cities)
        masked_dist_matrix = ma.masked_array(tempDM, mask=tempDM == 0)
        desired_val = masked_dist_matrix.min()
        next_nearest_1, next_nearest_2 = tuple(np.where(desired_val == dist_matrix_DF)[0][:2])
        if next_nearest_1 in tour:
            return next_nearest_2
        return next_nearest_1
    ...
    while len(tour) < n_cities + 2:
        next_wpt = _find_next_nearest(dist_matrix_DF, tour, list(city_ids))
        change_arc_list = [
            D_ijk(i, j, next_wpt, _d_ijk(i, j, next_wpt, dist_matrix))
            for i, j in zip(tour, tour[1:])
        ]
        change_arc_list.sort(key=lambda __x: __x.val)
        if change_arc_list:
            near_insert = change_arc_list.pop(0)
            while not (near_insert.k in city_ids):
                near_insert = change_arc_list.pop(0)
...
def construct_path_nearest_insertion_heuristic_v3(dist_matrix, start_min_arc=True, c_num=None):
    n_cities = len(dist_matrix)
    dist_matrix[:, 0] = 0
    rts = []
    for i in range(5):
        route, dist = solve_tsp_local_search(dist_matrix)
        rts.append(tuple([dist, route]))
    rts.sort()
    min_dist, min_route = rts[0]
